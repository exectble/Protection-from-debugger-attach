...
//Затем драйвер ядра выполняет процесс выборочного удаления разрешений (например, доступ для чтения к памяти процесса)
//перед передачей дескриптора запрашивающему процессу.
